...
export const getOne: IProductGetOne = async ({
  productId,
}: {
  productId: string;
}) => {
...
export const getAll: IProductGetAll = async ({
  pageSize,
}: {
  pageSize: number;
}) => {
...
export const getRelated: IProductGetRelated = async ({
  productId,
  pageSize,
}: {
  productId: string;
  ...
  const related = products.filter((product) => product.productId !== productId);
